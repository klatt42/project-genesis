...
export class SimilarityAnalyzer {
     ...
     */
    findSimilarProjects(graph, projectId, threshold = 0.5) {
     ...
     */
    calculateProjectSimilarity(graph, project1, project2) {
        const reasons = [];
        let score = 0;
        let maxScore = 0;
     ...
     */
    findSimilarPatterns(graph, patternId, threshold = 0.5) {
     ...
     */
    expandCluster(graph, startNode, visited) {
        const cluster = [];
        ...
        while (queue.length > 0) {
            ...
            for (const neighborId of neighbors) {
                if (visited.has(neighborId))
                    continue;
                const edge = Array.from(graph.edges.values()).find(e => e.sourceId === node.id && e.targetId === neighborId);
     ...
     */
    countSimilarPatterns(patterns1, patterns2) {
        let count = 0;
...
//# sourceMappingURL=similarity-analyzer.js.map