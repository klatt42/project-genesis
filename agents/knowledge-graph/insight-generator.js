...
export class InsightGenerator {
        ...
        for (const component of components) {
            ...
            if (usageRate > 0.5 && installations.length >= 3) {
                insights.push({
                    ...
                    relatedNodes: [component.id, ...installations.map((id) => `project-${id}`)],
                    createdAt: new Date()
                });
            }
        }
     ...
     * Find consolidation opportunities
     */
    findConsolidationOpportunities(graph) {
        const insights = [];
     ...
     */
    generateRecommendations(graph, projectId) {
        ...
        for (const similarProject of similarProjects.slice(0, 3)) {
            ...
            for (const pattern of patterns) {
                ...
                const hasPattern = currentPatterns.some(p => p.metadata?.type === pattern.metadata?.type &&
     ...
     */
    findSimilarProjectNodes(graph, project) {
        const allProjects = Array.from(graph.nodes.values())
        ...
        return allProjects
            ...
            const sharedTech = projectTech.filter(t => otherTech.some(ot => ot.id === t.id));
            return {
                project: other,
     ...
     */
    getConnectedNodes(graph, nodeId, type) {
