...
export class DashboardGenerator {
   ...
   */
  private generateHeader(aggregated: AggregatedMetrics): string {
    const width = 80;
   ...
   */
  private generatePortfolioOverview(aggregated: AggregatedMetrics): string {
    const lines: string[] = [];
   ...
   */
  private generateErrorsSection(aggregated: AggregatedMetrics): string {
    const lines: string[] = [];
   ...
   */
  private generateAnalyticsSection(aggregated: AggregatedMetrics): string {
    const lines: string[] = [];
   ...
   */
  private generatePerformanceSection(aggregated: AggregatedMetrics): string {
    const lines: string[] = [];
   ...
   */
  private generateUptimeSection(aggregated: AggregatedMetrics): string {
    const lines: string[] = [];
   ...
   */
  private generateProjectList(projects: ProjectMetrics[]): string {
    const lines: string[] = [];
   ...
   */
  private createBox(lines: string[], width: number): string {
  ...
  private getHealthBar(health: number): string {
    ...
    if (health >= 80) return this.colorize(health, bar, 'green');
    if (health >= 50) return this.colorize(health, bar, 'yellow');
   ...
   */
  private colorize(value: any, text: string, color: 'green' | 'yellow' | 'red'): string {
