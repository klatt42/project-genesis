...
export class AlertCoordinator {
   ...
   */
  async checkAlerts(metrics: ProjectMetrics[]): Promise<Alert[]> {
      ...
      for (const rule of this.rules.values()) {
        ...
        if (alert) {
          ...
          const existingAlert = Array.from(this.alerts.values()).find(
            a => a.projectId === metric.projectId &&
                 a.type === rule.type &&
                 !a.resolved
          );
   ...
   */
  private evaluateRule(rule: AlertRule, metrics: ProjectMetrics): Alert | null {
    let value: number = 0;
    let message: string = '';
   ...
   */
  private meetsThreshold(value: number, condition: AlertRule['condition']): boolean {
  ...
  private async loadRules(): Promise<void> {
    try {
      ...
      this.rules = new Map(parsed.rules.map((r: AlertRule) => [r.id, r]));
    } catch (error) {
