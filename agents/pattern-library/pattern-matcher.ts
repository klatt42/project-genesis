...
export class PatternMatcher {
  ...
  private calculateSimilarity(pattern1: Pattern, pattern2: Pattern): number {
    ...
    // Same type (weight: 3)
    maxScore += 3;
    if (pattern1.type === pattern2.type) score += 3;

    // Same category (weight: 2)
    maxScore += 2;
    if (pattern1.category === pattern2.category) score += 2;

    // Shared keywords (weight: 0.5 per keyword, max 3)
    maxScore += 3;
    const sharedKeywords = pattern1.keywords.filter(k =>
    ...
    maxScore += 2;
    const sharedDeps = pattern1.dependencies.filter(d =>
      pattern2.dependencies.includes(d)
    );
   ...
   */
  private calculateRelevance(keywords: string[], pattern: Pattern): number {
    let score = 0;
    let matches = 0;
   ...
   */
  private generateMatchReason(pattern1: Pattern, pattern2: Pattern, score: number): string {
   ...
   */
  private generateSuggestionReason(keywords: string[], pattern: Pattern, relevance: number): string {
    const matchedKeywords = keywords.filter(k =>
      pattern.keywords.some(pk => pk.includes(k) || k.includes(pk)) ||
   ...
   */
  private generateExampleUsage(pattern: Pattern): string {
  ...
  findPatternsUsedTogether(
    ...
    for (const otherPattern of allPatterns) {
      ...
      const sharedDeps = pattern.dependencies.filter(d =>
        otherPattern.dependencies.includes(d)
      );

      if (sharedDeps.length > 0) {
        coOccurrence.set(
          otherPattern.id,
          (coOccurrence.get(otherPattern.id) || 0) + sharedDeps.length
        );
      }
    }
  ...
  findReplacements(
    pattern: Pattern,
    allPatterns: Pattern[]
  ): Array<{ pattern: Pattern; reason: string }> {
    const replacements: Array<{ pattern: Pattern; reason: string }> = [];
