...
export class PatternLibrary {
  ...
  private indexer: PatternIndexer;
  ...
  async extractFromProject(
    ...
    const result = await this.extractor.extractFromProject(
      projectPath,
      projectId,
      projectName
    );
   ...
   * Search patterns
   */
  async search(query: PatternSearchQuery): Promise<Pattern[]> {
   ...
   */
  async suggest(requirement: string, limit?: number): Promise<PatternSuggestion[]> {
   ...
   */
  async findSimilar(patternId: string, threshold?: number): Promise<PatternMatch[]> {
   ...
   */
  getHighestQuality(limit?: number) {
