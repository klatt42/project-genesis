...
import { PatternType, PatternCategory } from './types.js';
export class PatternExtractor {
        ...
        try {
            ...
            for (const dir of dirsToScan) {
                ...
                if (existsSync(fullPath)) {
                    const extracted = await this.scanDirectory(fullPath, projectPath, projectId, projectName);
            ...
            const configPatterns = await this.extractConfigPatterns(projectPath, projectId, projectName);
            patterns.push(...configPatterns);
        }
        catch (error) {
            errors.push(`Error extracting patterns: ${error instanceof Error ? error.message : String(error)}`);
        }
     ...
     */
    async scanDirectory(dirPath, projectPath, projectId, projectName) {
        const patterns = [];
        const errors = [];
        let filesScanned = 0;
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
                    ...
                    filesScanned++;
                    try {
                        const pattern = await this.extractPattern(fullPath, projectPath, projectId, projectName);
                        if (pattern) {
                            patterns.push(pattern);
                        }
                    }
                    catch (error) {
                        errors.push(`Error extracting ${fullPath}: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
            }
        }
        catch (error) {
            errors.push(`Error scanning ${dirPath}: ${error instanceof Error ? error.message : String(error)}`);
        }
     ...
     */
    async extractPattern(filePath, projectPath, projectId, projectName) {
     ...
     * Extract configuration patterns
     */
    async extractConfigPatterns(projectPath, projectId, projectName) {
        const patterns = [];
        const configFiles = [
            'tailwind.config.ts',
            'tailwind.config.js',
     ...
     */
    determinePatternType(filePath, code) {
     ...
     */
    determinePatternCategory(filePath, code) {
    ...
    calculateComplexity(code) {
        ...
        const complexity = Math.min(10, Math.round((lines / 100) + (functions / 5) + (conditions / 10) + (loops / 5)));
        return Math.max(1, complexity);
    }
    /**
     * Calculate code quality (1-10)
     */
    calculateQuality(code, type) {
     ...
     * Generate pattern ID
     */
    generatePatternId(projectId, filePath) {
