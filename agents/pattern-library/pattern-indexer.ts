...
export class PatternIndexer {
  ...
  constructor(indexPath?: string) {
    this.indexPath = indexPath || path.join(
      process.env.HOME || '',
      '.genesis',
      'portfolio',
      'patterns.json'
  ...
  searchPatterns(query: PatternSearchQuery): Pattern[] {
    let results: Pattern[] = Array.from(this.index.patterns.values());
   ...
   */
  getHighestQualityPatterns(limit: number = 10): Pattern[] {
    const patterns = this.getAllPatterns();
    return patterns
   ...
   */
  async clearProjectPatterns(projectId: string): Promise<void> {
  ...
  private async load(): Promise<void> {
    try {
      ...
      this.index.byType = new Map(Object.entries(parsed.byType || {}) as [PatternType, string[]][]);
      this.index.byCategory = new Map(Object.entries(parsed.byCategory || {}) as [PatternCategory, string[]][]);
