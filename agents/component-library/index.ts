...
import { VersionManager, type UpdateCheck } from './version-manager.js';
import type {
  Component,
...
export class ComponentLibrary {
  ...
  private packager: ComponentPackager;
   ...
   */
  async extractComponent(component: Component): Promise<void> {
   ...
   */
  async packageComponent(componentId: string): Promise<ComponentPackage> {
    await this.ensureInitialized();
  ...
  async installComponent(
    ...
    if (options.version) {
      const versionExists = component.versions.some(
        v => v.version === options.version
      );
      if (!versionExists) {
        throw this.createError(
          `Version ${options.version} not found for ${componentName}`,
          'VERSION_NOT_FOUND'
        );
      }
  ...
  async publishVersion(
    ...
    const nextVersion = this.versionManager.suggestNextVersion(
      component.version,
      breaking,
      true
    );
   ...
   */
  async searchComponents(query: ComponentSearchQuery): Promise<Component[]> {
    await this.ensureInitialized();
   ...
   */
  async getUsageStats(componentName: string) {
    await this.ensureInitialized();
   ...
   */
  async getPackageInfo(componentId: string): Promise<ComponentPackage | null> {
    await this.ensureInitialized();
   ...
   * Create error
   */
  private createError(message: string, code: string, details?: any): ComponentLibraryError {
    const error = new Error(message) as ComponentLibraryError;
    error.code = code;
...
export async function extractComponent(component: Component): Promise<void> {
...
export async function searchComponents(query: ComponentSearchQuery): Promise<Component[]> {
