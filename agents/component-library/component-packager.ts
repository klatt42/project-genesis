...
export class ComponentPackager {
  ...
  constructor(packagesPath?: string) {
    this.packagesPath = packagesPath || path.join(
      process.env.HOME || '',
      '.genesis',
      'portfolio',
      'packages'
   ...
   * Package a component
   */
  async packageComponent(component: Component): Promise<ComponentPackage> {
    ...
    if (examples.length > 0) {
      ...
      for (let i = 0; i < examples.length; i++) {
        await fs.writeFile(
          path.join(examplesDir, `example-${i + 1}.tsx`),
          examples[i],
          'utf-8'
        );
      }
   ...
   */
  private generateReadme(component: Component): string {
    const lines: string[] = [];
   ...
   */
  private generatePackageJson(component: Component) {
    return {
   ...
   */
  private extractExamples(component: Component): string[] {
   ...
   */
  private generateBasicExample(component: Component): string {
   ...
   */
  private getExportName(component: Component): string {
   ...
   */
  async isPackaged(componentId: string): Promise<boolean> {
   ...
   * Remove package
   */
  async removePackage(componentId: string): Promise<void> {
    const packagePath = this.getPackagePath(componentId);
    if (existsSync(packagePath)) {
