...
export class ComponentInstaller {
     ...
     */
    async install(component, options) {
        const errors = [];
        const warnings = [];
        ...
        try {
            ...
            if (options.installDependencies) {
                const depResult = await this.installDependencies(component, options.targetPath);
     ...
     */
    async checkConflicts(component, targetPath) {
        const conflicts = [];
        for (const file of component.files) {
     ...
     */
    async updateImports(component, targetPath) {
            ...
            try {
                ...
                while ((match = importRegex.exec(content)) !== null) {
                    ...
                    if (importPath.includes(component.name)) {
                        ...
                        const newImportPath = this.calculateRelativeImport(projectFile, path.join(targetPath, component.mainFile));
                        const oldImportStatement = match[0];
                        const newImportStatement = oldImportStatement.replace(importPath, newImportPath);
     ...
     */
    calculateRelativeImport(from, to) {
     ...
     */
    async installDependencies(component, targetPath) {
        const errors = [];
        try {
     ...
     */
    async uninstall(component, targetPath) {
        const errors = [];
        const warnings = [];
        const removedFiles = [];
        try {
