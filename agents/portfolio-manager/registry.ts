...
export class ProjectRegistry {
  ...
  private relationships: ProjectRelationship[] = [];
   ...
   * Register a project
   */
  async registerProject(projectPath: string): Promise<ProjectMetadata> {
    // Scan project
  ...
  async removeProject(projectId: string): Promise<void> {
    ...
    this.relationships = this.relationships.filter(
      r => r.sourceProject !== projectId && r.targetProject !== projectId
    );
  ...
  searchProjects(criteria: ProjectSearchCriteria): ProjectMetadata[] {
    return this.getAllProjects().filter(project => {
      if (criteria.type && project.type !== criteria.type) return false;
      if (criteria.status && project.status !== criteria.status) return false;
      if (criteria.framework && project.stack.framework !== criteria.framework) return false;
      if (criteria.deploymentPlatform && project.stack.deployment !== criteria.deploymentPlatform) return false;
      if (criteria.hasPattern && !project.patterns.includes(criteria.hasPattern)) return false;
      if (criteria.hasComponent && !project.components.includes(criteria.hasComponent)) return false;
      if (criteria.minUptime !== undefined && project.uptime < criteria.minUptime) return false;
      if (criteria.maxErrorRate !== undefined && project.errorRate > criteria.maxErrorRate) return false;
   ...
   */
  async addRelationship(relationship: ProjectRelationship): Promise<void> {
    // Check if relationship already exists
    const exists = this.relationships.some(
      r =>
        r.sourceProject === relationship.sourceProject &&
  ...
  private async scanProject(projectPath: string): Promise<ProjectScanResult> {
    try {
      ...
      const isGenesisProject =
        packageJson.dependencies?.['@genesis/core'] ||
      ...
      // Detect framework
      const stack: Partial<ProjectMetadata['stack']> = {
